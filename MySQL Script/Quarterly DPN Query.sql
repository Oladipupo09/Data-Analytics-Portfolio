...
CREATE TABLE `DPN_MASTER`

AS

#(

INSERT INTO  `Setup_dpn_Q122` (contract_id, Contract, ReportDate, generation, `type`, ltoperiod, contract_creation,
...
rev_share,`year`, amount_after_rev_share)
...
WHEN ltoperiod IN ('12','24') THEN 'LCP'
...
paidperiod, old_active_status,'2022-03-31' dpn_date
FROM activity_mar22 
WHERE TYPE <> 'Outright' 
)a
LEFT JOIN (
...
CREATE TABLE dpn_check
AS
SELECT Contract, ReportDate, sales_channel, generation, `TYPE`, contract_creation,  crmcontract, old_active_status, dpn_date, 
ltoperiod, paidperiod,  contract_id, initial_down_payment, contract_end_date, `Set-Up_without_VAT`,
Inclusion_Check, Datediff1/Datediff2 Check2
FROM
(SELECT a.Contract, a.ReportDate, a.sales_channel, a.generation, a.TYPE, a.ltoperiod, a.contract_creation, a.crmcontract,
a.paidperiod, a.old_active_status,a.dpn_date, b.contract_id, IFNULL(b.initial_down_payment,0) initial_down_payment, b.contract_end_date, 
CASE WHEN a.contract_creation >= '2020-02-01' THEN IFNULL((b.initial_down_payment/1.075),0)
ELSE IFNULL((b.initial_down_payment/1.05),0) END `Set-Up_without_VAT`,
CASE WHEN a.old_active_status IN ('Cancelled','Owner') THEN 1 ELSE NULL END Inclusion_Check,
ROUND(IFNULL(DATEDIFF(a.dpn_date,a.contract_creation),0),2) Datediff1, 
CASE WHEN `type` = 'RENTAL' THEN 48 * 30.4
WHEN a.generation = 'UNIFIED' THEN a.ltoperiod * 30.4 ELSE a.ltoperiod END Datediff2
FROM (
(
SELECT Contract, ReportDate, sales_channel, generation, `type`, ltoperiod, contract_creation, crmcontract,
paidperiod, old_active_status,'2022-03-31' dpn_date
...
DROP TABLE dpn_check

SELECT DISTINCT(generation), ltoperiod, TYPE FROM `activity_mar22`