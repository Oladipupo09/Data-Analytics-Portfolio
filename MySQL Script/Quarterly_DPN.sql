...
CASE WHEN a.contract_creation >= '2020-02-01' THEN IFNULL((b.initial_down_payment/1.075),0) #vat change date 2020-02-01
...
paidperiod, old_active_status,'2023-03-31' dpn_date
FROM `activity_log` 
WHERE `month_date` = 'Mar-23' AND TYPE <> 'Outright' 
)a
LEFT JOIN (
SELECT `full_sfid`,contract_id,sales_channel,initial_down_payment,warranty_end_date contract_end_date
...
SELECT DISTINCT`contract`,`light_status`
FROM `activity_log`
WHERE `month_date` = 'Apr-23' AND 

SELECT * FROM mtn_pos_ii


/*INSERT INTO mtn_pos_ii (contract_id,POS)
